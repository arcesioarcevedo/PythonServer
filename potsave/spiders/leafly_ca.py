...
class LeaflySpiderCA(scrapy.Spider):
    name = "leaflyca"
    scrap1 = []
    ...
    def start_requests(self):
        url = "https://finder-service.leafly.com/v3/dispensaries?filter[]=dispensary&geo_query_type=point&promote_new_stores=true&lat=56.130366&limit=20&lon=-106.346771&page=1&radius=1608mi&return_facets=true&sort=default&sort_version=default"
        ...
        headers = {
            ...
            "origin": "https://www.leafly.ca",
            "referer": "https://www.leafly.ca/",
        ...
        yield scrapy.Request(
            url=url, headers=headers, callback=self.urls_iterate, dont_filter=True
        )

    def urls_iterate(self, response):
        print('here1')
        count = json.loads(response.text)["pageCount"]
        for page in range(1, count + 1):
            url = (
                "https://finder-service.leafly.com/v3/dispensaries?filter[]=dispensary&geo_query_type=point&promote_new_stores=true&lat=56.130366&limit=20&lon=-106.346771&page="
                + str(page)
    ...
    def dsp_iterate(self, response):
        ...
        for store in stores:
            url = "https://www.leafly.ca/cannabis-store/" + store["slug"]
            headers = {
                ...
                "dnt": "1",
                "referer": "https://www.leafly.ca/dispensaries",
    ...
    def dsp_data(self, response):
        ...
        add_dsp1 = "INSERT INTO Products_info (log_data, source, created_at, updated_at) VALUES (%s, %s, %s, %s)"
        self.cursor.executemany(add_dsp1, scrap3_except)
        self.cnx.commit()
    ...
    def pages_data(self, response):
        try:
            ...
            # self.cnx.commit()
            temp_turpl1 = (log_data, "leafly", str(datetime.datetime.now()), str(datetime.datetime.now()))
            scrap3_except.append(temp_turple1)
    ...
    def product_urls(self, response):
        ...
        if products:
            ...
            self.cursor.executemany(add_dsp, prodct_luist_comp)
            self.cnx.commit()
